import until from '../utils/untilNow';

export default function render(place, data, placeOpt = 'afterbegin') {
  const $joke = `
          <div class="joke" data-id="${data.id}">
              <div class="joke__content">
                  <div class="joke__icon">
                  ...
                  <div class="joke__text">
                      <div class="joke__link">
                          <span>ID:<a href="${data.url}" class="joke__ancor">${data.url} <svg width="10" height="10"
                          ...
                          <div class="joke__like ${data.liked ? 'joke__like_active' : ''}">
                              <svg width="20" height="17" class="disliked" viewBox="0 0 20 17" fill="none"
                                  xmlns="http://www.w3.org/2000/svg">
                                  <path
                              ...
                              <svg width="20" height="17" viewBox="0 0 20 17" class="liked" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <p class="joke__value">${data.value}</p>
                      <div class="joke__data">
                          <div class="joke__update">
                            Last update: <span class="joke__date" data-time="${data.updated_at}">${until(data.updated_at)} hours ago</span>
                          </div>
                          ${data.categories.length !== 0 ? `<div class="joke__category">${data.categories}</div>` : '<div class="joke__category"></div>'}
                      </div>
                  </div>
              </div>
          </div>
      `;
  place.insertAdjacentHTML(placeOpt, $joke);
}
